...
Detaljpanel (FDetailsPanel) används tillsammand med {@link FPageLayout Applikationslayout} för att visa detaljer om ett av användaren valt objekt.
...
Innehåll läggs i respektive slot:
...
Namned man anget i propen `name` används som identifierare för att hämta ut rätt detaljpanel.
...
Det är rekommenderat att lägga namnet på panelen i en konstant som återanvänds mellan filer.
...
Objektet kan återfås i panelen med scope attribute `item`:
...
Om konsumenten vill agera på att detaljpanelen stängs kan man skicka en valfri callback:
...
Använder du [HTML-validate][html-validate] rekommenderas att definiera upp att elementet ärver från `FDetailsPanel`:
...
Konsumenten kan lägga in egna knappar som stänger detaljpanelen.
...
Som en riktline rekommenderar vi att endast visa en detaljpanel åt gången.
