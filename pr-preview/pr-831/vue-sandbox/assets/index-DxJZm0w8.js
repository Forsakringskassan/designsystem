...
class ValidationServiceImpl {
  ...
  validateAll(element, validationState, validators, validatorConfigs) {
    ...
    const untrimmedValue = this.getUntrimmedValue(element);
    const resultFromValidators = validators.map((validator) => {
      const elementValue = validator.name === "whitelist" ? untrimmedValue : value;
      return {
        isValid: this.validate(elementValue, element, validator, validatorConfigs),
        name: validator.name
      };
    });
