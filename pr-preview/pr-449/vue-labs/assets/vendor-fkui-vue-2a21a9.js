...
function formatDateRange(el, range) {
  ...
  const fromDate = range.from instanceof FDate ? range.from.toString() : parseDate(range.from);
  const toDate = range.to instanceof FDate ? range.to.toString() : parseDate(range.to);
  if (fromDate && toDate) {
    el.textContent = `${FDate.fromIso(fromDate !== null && fromDate !== void 0 ? fromDate : "")} \u2013 ${FDate.fromIso(toDate !== null && toDate !== void 0 ? toDate : "")}`;
  }
}
