...
var camelize$1 = cacheStringFunction$2((str) => {
	return str.replace(camelizeRE$2, (c) => c.slice(1).toUpperCase());
});
var hyphenateRE$2 = /\B([A-Z])/g;
var hyphenate$1 = cacheStringFunction$2((str) => str.replace(hyphenateRE$2, "-$1").toLowerCase());
...
function normalizeStyle(value) {
	if (isArray$1(value)) {
		...
		return res;
	} else if (isString$1(value) || isObject$3(value)) return value;
}
...
var toDisplayString = (val) => {
	return isString$1(val) ? val : val == null ? "" : isArray$1(val) || isObject$3(val) && (val.toString === objectToString || !isFunction(val.toString)) ? isRef$1(val) ? toDisplayString(val.value) : JSON.stringify(val, replacer, 2) : String(val);
};
var replacer = (_key, val) => {
	...
	else if (isObject$3(val) && !isArray$1(val) && !isPlainObject(val)) return String(val);
	return val;
};
...
var BaseReactiveHandler = class {
	...
	get(target, key, receiver) {
		...
		if (isObject$3(res)) return isReadonly2 ? /* @__PURE__ */ readonly(res) : /* @__PURE__ */ reactive(res);
		return res;
	}
};
...
var toReactive = (value) => isObject$3(value) ? /* @__PURE__ */ reactive(value) : value;
var toReadonly = (value) => isObject$3(value) ? /* @__PURE__ */ readonly(value) : value;
/* @__NO_SIDE_EFFECTS__ */
function isRef(r) {
...
function toRef(source, key, defaultValue) {
	...
	else if (isObject$3(source) && arguments.length > 1) return propertyToRef(source, key, defaultValue);
...
var TeleportImpl = {
	...
	remove(vnode, parentComponent, parentSuspense, { um: unmount, o: { remove: hostRemove } }, doRemove) {
		...
		if (shapeFlag & 16) {
			const shouldRemove = doRemove || !isTeleportDisabled$1(props);
...
function renderList(source, renderItem, cache, index) {
	...
	} else if (isObject$3(source)) if (source[Symbol.iterator]) ret = Array.from(source, (item, i) => renderItem(item, i, void 0, cached && cached[i]));
	else {
...
function applyOptions(instance) {
	...
	if (dataOptions) {
		...
		if (!isObject$3(data)) {} else instance.data = /* @__PURE__ */ reactive(data);
	}
...
function resolveInjections(injectOptions, ctx, checkDuplicateProperties = NOOP) {
	...
	for (const key in injectOptions) {
		...
		let injected;
		if (isObject$3(opt)) if ("default" in opt) injected = inject(opt.from || key, opt.default, true);
...
function createWatcher(raw, ctx, publicThis, key) {
	...
	else if (isObject$3(raw)) if (isArray$1(raw)) raw.forEach((r) => createWatcher(r, ctx, publicThis, key));
	else {
...
function resolveMergedOptions(instance) {
	...
	if (isObject$3(base)) cache.set(base, resolved);
	return resolved;
}
...
function createAppAPI(render, hydrate) {
	return function createApp(rootComponent, rootProps = null) {
		...
		if (rootProps != null && !isObject$3(rootProps)) rootProps = null;
...
function normalizeEmitsOptions(comp, appContext, asMixin = false) {
	...
	if (!raw && !hasExtends) {
		if (isObject$3(comp)) cache.set(comp, null);
		return null;
	}
	if (isArray$1(raw)) raw.forEach((key) => normalized[key] = null);
	else extend(normalized, raw);
	if (isObject$3(comp)) cache.set(comp, normalized);
	return normalized;
}
...
function hasPropValueChanged(nextProps, prevProps, key) {
	...
	if (key === "style" && isObject$3(nextProp) && isObject$3(prevProp)) return !looseEqual(nextProp, prevProp);
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
	...
	if (!raw && !hasExtends) {
		if (isObject$3(comp)) cache.set(comp, EMPTY_ARR);
		return EMPTY_ARR;
	}
	...
	if (isObject$3(comp)) cache.set(comp, res);
	return res;
}
...
function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
	...
	const shapeFlag = isString$1(type) ? 1 : isSuspense(type) ? 128 : isTeleport(type) ? 64 : isObject$3(type) ? 4 : isFunction(type) ? 2 : 0;
...
function handleSetupResult(instance, setupResult, isSSR) {
	...
	else if (isObject$3(setupResult)) instance.setupState = proxyRefs(setupResult);
...
function h(type, propsOrChildren, children) {
	try {
		...
		if (l === 2) if (isObject$3(propsOrChildren) && !isArray$1(propsOrChildren)) {
...
var _locale$1 = /* @__PURE__ */ getDefaultLocale$1();
/**
* Get current locale.
*
...
var FDate$1 = class FDate$1 {
	...
	*/
	get monthName() {
		if (this.isValid()) return this.value.locale(getLocale$1()).format("MMMM");
		else return "";
	}
	/**
	...
	*/
	get monthNameShort() {
		if (this.isValid()) return this.value.locale(getLocale$1()).format("MMM");
		else return "";
	}
	/**
	...
	*/
	get dayName() {
		if (this.isValid()) return this.value.locale(getLocale$1()).format("dddd");
		else return "";
	}
	/**
	...
	*/
	get dayNameShort() {
		if (this.isValid()) return this.value.locale(getLocale$1()).format("ddd");
		else return "";
	}
	/**
	...
	*/
	startOfMonth() {
		return new FDate$1(this.value.startOf("month"));
	}
	/**
	...
	*/
	endOfMonth() {
		return new FDate$1(this.value.endOf("month"));
	}
	/**
	...
	*/
	addDays(value) {
		return new FDate$1(this.value.add(value, "day"));
	}
	/**
	...
	*/
	addMonths(value) {
		return new FDate$1(this.value.add(value, "month"));
	}
	/**
	...
	*/
	addYears(value) {
		return new FDate$1(this.value.add(value, "year"));
	}
	/**
	...
	*/
	equals(rhs) {
		if (typeof rhs === "string") rhs = FDate$1.fromIso(rhs);
	...
	*/
	static compare(a, b) {
		if (typeof a === "string") a = FDate$1.fromIso(a);
		if (typeof b === "string") b = FDate$1.fromIso(b);
	...
	*/
	toString(format = DateFormat$1.ISO8601) {
		if (this.isValid()) {
			const template = formatter$1[getLocale$1()][format];
			return this.value.locale(getLocale$1()).format(template);
		} else return "";
	}
	/**
...
function parseDate(value) {
	...
	const date = FDate$1.fromIso(normalized);
...
* @public
*/
function parsePersonnummer(value, now = FDate$1.now()) {
	...
	if (!isValidDate(FDate$1.fromYearMonthDay(century + year, month, dayWithoutSamordning), now)) return;
...
function formatPersonnummer(value) {
	if (!isSet(value)) return;
	if (FDate$1.now().year - Number(value.slice(0, 4)) >= 100) return value.slice(2).replace("-", "+");
...
var blacklistValidator = {
	...
	validation(value, _element, config) {
		...
		return !toArray$2(config.values).some((it) => String(it) === value);
	}
};
...
var dateValidator = {
	...
	validation(value) {
		...
		return FDate$1.fromIso(normalized).isValid();
	}
};
...
var maxDateValidator = {
	...
	validation(value, _element, config) {
		...
		const limit = FDate$1.fromIso(validLimit(config.limit));
...
var require_is_possible_prototype$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var isObject = require_is_object$1();
	module.exports = function(argument) {
		return isObject(argument) || argument === null;
	};
}));
var require_a_possible_prototype$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var isPossiblePrototype = require_is_possible_prototype$1();
	var $String = String;
	var $TypeError = TypeError;
	module.exports = function(argument) {
...
var require_object_set_prototype_of$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var uncurryThisAccessor = require_function_uncurry_this_accessor$1();
	var isObject = require_is_object$1();
	var requireObjectCoercible = require_require_object_coercible$1();
	var aPossiblePrototype = require_a_possible_prototype$1();
...
var require_array_buffer_view_core$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var NATIVE_ARRAY_BUFFER = require_array_buffer_basic_detection$1();
	var DESCRIPTORS = require_descriptors$1();
	var globalThis = require_global_this$1();
	var isCallable = require_is_callable$1();
	...
	var setPrototypeOf = require_object_set_prototype_of$1();
	var wellKnownSymbol = require_well_known_symbol$1();
	var uid = require_uid$1();
	var InternalStateModule = require_internal_state$1();
	var enforceInternalState = InternalStateModule.enforce;
...
var require_es_typed_array_to_reversed$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var lengthOfArrayLike = require_length_of_array_like$1();
	var ArrayBufferViewCore = require_array_buffer_view_core$1();
...
var require_es_typed_array_to_sorted$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var ArrayBufferViewCore = require_array_buffer_view_core$1();
	var uncurryThis = require_function_uncurry_this$1();
	var aCallable = require_a_callable$1();
...
var require_is_big_int_array$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var classof = require_classof$1();
	module.exports = function(it) {
		var klass = classof(it);
...
var require_to_big_int$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var toPrimitive = require_to_primitive$1();
	var $TypeError = TypeError;
	module.exports = function(argument) {
...
var require_es_typed_array_with$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var ArrayBufferViewCore = require_array_buffer_view_core$1();
	var isBigIntArray = require_is_big_int_array$1();
	var lengthOfArrayLike = require_length_of_array_like$1();
	var toIntegerOrInfinity = require_to_integer_or_infinity$1();
	var toBigInt = require_to_big_int$1();
	var aTypedArray = ArrayBufferViewCore.aTypedArray;
	var getTypedArrayConstructor = ArrayBufferViewCore.getTypedArrayConstructor;
	var exportTypedArrayMethod = ArrayBufferViewCore.exportTypedArrayMethod;
	var $RangeError = RangeError;
...
var require_an_object_or_undefined$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var isObject = require_is_object$1();
	var $String = String;
	var $TypeError = TypeError;
	module.exports = function(argument) {
...
var require_a_string$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var $TypeError = TypeError;
	module.exports = function(argument) {
...
var require_base64_map$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
...
var require_get_alphabet_option$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var $TypeError = TypeError;
...
var require_uint8_from_base64$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var globalThis = require_global_this$1();
	var uncurryThis = require_function_uncurry_this$1();
	var anObjectOrUndefined = require_an_object_or_undefined$1();
	var aString = require_a_string$1();
	var hasOwn = require_has_own_property$1();
	var base64Map = require_base64_map$1();
	var getAlphabetOption = require_get_alphabet_option$1();
	var notDetached = require_array_buffer_not_detached$1();
...
var require_an_uint8_array$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var classof = require_classof$1();
	var $TypeError = TypeError;
	module.exports = function(argument) {
...
var require_es_uint8_array_set_from_base64$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var globalThis = require_global_this$1();
	var $fromBase64 = require_uint8_from_base64$1();
	var anUint8Array = require_an_uint8_array$1();
...
var require_uint8_from_hex$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var globalThis = require_global_this$1();
	var uncurryThis = require_function_uncurry_this$1();
	var Uint8Array = globalThis.Uint8Array;
	var SyntaxError = globalThis.SyntaxError;
...
var require_es_uint8_array_set_from_hex$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var globalThis = require_global_this$1();
	var aString = require_a_string$1();
	var anUint8Array = require_an_uint8_array$1();
	var notDetached = require_array_buffer_not_detached$1();
	var $fromHex = require_uint8_from_hex$1();
...
var require_es_uint8_array_to_base64$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var globalThis = require_global_this$1();
	var uncurryThis = require_function_uncurry_this$1();
	var anObjectOrUndefined = require_an_object_or_undefined$1();
...
var require_es_uint8_array_to_hex$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var globalThis = require_global_this$1();
	var uncurryThis = require_function_uncurry_this$1();
	var anUint8Array = require_an_uint8_array$1();
...
require_es_typed_array_to_reversed$1();
require_es_typed_array_to_sorted$1();
require_es_typed_array_with$1();
require_es_uint8_array_set_from_base64$1();
require_es_uint8_array_set_from_hex$1();
require_es_uint8_array_to_base64$1();
require_es_uint8_array_to_hex$1();
...
var require_is_array$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var classof = require_classof_raw$1();
	module.exports = Array.isArray || function isArray(argument) {
		return classof(argument) === "Array";
	};
}));
var require_array_set_length$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var DESCRIPTORS = require_descriptors$1();
	var isArray = require_is_array$1();
	var $TypeError = TypeError;
	var getOwnPropertyDescriptor = Object.getOwnPropertyDescriptor;
...
var require_does_not_exceed_safe_integer$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var $TypeError = TypeError;
	var MAX_SAFE_INTEGER = 9007199254740991;
	module.exports = function(it) {
...
var require_es_array_push = /* @__PURE__ */ __commonJSMin$1((() => {
	...
	var toObject = require_to_object$1();
	var lengthOfArrayLike = require_length_of_array_like$1();
	var setArrayLength = require_array_set_length$1();
	var doesNotExceedSafeInteger = require_does_not_exceed_safe_integer$1();
...
var require_es_set_union_v2 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
...
var require_es_iterator_map = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var call = require_function_call$1();
	var aCallable = require_a_callable$1();
	var anObject = require_an_object$1();
...
var IPopup_vue_vue_type_script_lang_default = /* @__PURE__ */ defineComponent({
	...
	methods: {
		...
		applyFocus() {
			...
			if (!wrapper) return;
			this.focus = pushFocus(getFocusableElement(wrapper, this.focusElement));
		},
		isMobileSize() {
...
var IPopup_default = /* @__PURE__ */ _plugin_vue_export_helper_default$1(IPopup_vue_vue_type_script_lang_default, [["render", _sfc_render$35]]);
...
var IPopupListbox_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		useEventListener$2(__props.anchor, "keyup", onKeyEsc);
		watchEffect(() => {
...
function useCombobox(inputRef, options, onOptionSelected) {
	...
	useEventListener$2(inputRef, "click", onInputClick);
	useEventListener$2(inputRef, "focus", onInputFocus);
	useEventListener$2(inputRef, "keydown", onInputKeyDown);
	useEventListener$2(inputRef, "keyup", onInputKeyUp);
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$48, [createVNode(unref(IPopupListbox_default), {
				...
				default: withCtx(() => [createBaseVNode("ul", {
					...
					}, toDisplayString(item), 11, _hoisted_3$28);
				}), 128))], 8, _hoisted_2$36)]),
				_: 1
			}, 8, [
				"is-open",
...
var _hoisted_3$24 = ["onClick"];
var _hoisted_4$20 = ["tabindex"];
var _hoisted_5$16 = {
	key: 0,
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(IPopup_default), {
				...
				default: withCtx(() => [createBaseVNode("nav", {
					...
					onKeyup: onKeyUp,
					onKeydown: onKeyDown
				}, [createBaseVNode("ul", _hoisted_2$32, [(openBlock(true), createElementBlock(Fragment, null, renderList(popupItems.value, (item, index) => {
					return openBlock(), createElementBlock("li", {
						key: item.key,
						role: "menuitem",
						...
						_cache[2] || (_cache[2] = createTextVNode()),
						hasSeparatorAfterItemAt(index) ? (openBlock(), createElementBlock("hr", _hoisted_5$16)) : createCommentVNode("", true)
					], 8, _hoisted_3$24);
				}), 128))], 512)], 40, _hoisted_1$43)]),
				_: 1
			}, 8, ["is-open", "anchor"]);
		};
	}
...
function _sfc_render$26(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return _ctx.$slots.tooltip ? (openBlock(), createElementBlock("div", _hoisted_1$40, [
		...
		}, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_3$23)], 512)) : createCommentVNode("", true),
		_cache[2] || (_cache[2] = createTextVNode()),
...
function _sfc_render$25(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createElementBlock("div", {
		...
		createBaseVNode("div", { class: normalizeClass(["select-field__icon-wrapper", _ctx.selectWrapperClass]) }, [
			...
			}, _ctx.attrs), [renderSlot(_ctx.$slots, "default")], 16, _hoisted_1$39), [[vModelSelect, _ctx.vModel]]),
			_cache[5] || (_cache[5] = createTextVNode()),
		...
		createBaseVNode("div", { class: normalizeClass(_ctx.labelWrapperClass) }, [createVNode(_component_f_label, {
			...
			description: withCtx(({ descriptionClass, formatDescriptionClass }) => [renderSlot(_ctx.$slots, "description", {
				...
				_ctx.discreteDescriptionText ? (openBlock(), createElementBlock("span", {
					...
					_ctx.discreteDescriptionScreenReaderText ? (openBlock(), createElementBlock("span", _hoisted_3$22, toDisplayString(_ctx.discreteDescriptionScreenReaderText), 1)) : createCommentVNode("", true),
...
var require_get_built_in_prototype_method$1 = /* @__PURE__ */ __commonJSMin$1(((exports, module) => {
	var globalThis = require_global_this$1();
...
(/* @__PURE__ */ __commonJSMin$1((() => {
	...
	var toIndexedObject = require_to_indexed_object$1();
	var arrayFromConstructorAndList = require_array_from_constructor_and_list$1();
	var getBuiltInPrototypeMethod = require_get_built_in_prototype_method$1();
	var addToUnscopables = require_add_to_unscopables$1();
	var $Array = Array;
...
function sortFilterData(data, filterAttributes, searchString, sortAttribute) {
	return sort(filter(data, filterAttributes, searchString), {
		attribute: sortAttribute.attribute,
		ascending: sortAttribute.ascending
	});
}
function useSortFilterDataset(data, sortableAttributes, filterAttributes, defaultSortAttribute, defaultSortAscending) {
								...
								default: withCtx(() => [
									...
									__props.showSort ? (openBlock(), createBlock(unref(IFlexItem_default), {
										...
										default: withCtx(() => [createVNode(unref(FSelectField_default), {
											...
											default: withCtx(() => [
												_cache[2] || (_cache[2] = createTextVNode()),
												createBaseVNode("option", { value: unref(defaultSortValue) }, toDisplayString(unref($t)("fkui.sort-filter-dataset.label.unsorted", "Välj")), 9, _hoisted_3$21),
												_cache[3] || (_cache[3] = createTextVNode()),
...
var ITableExpandButton_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return __props.isExpandable ? (openBlock(), createElementBlock("td", _hoisted_1$14, [createBaseVNode("button", {
				ref: "expandable",
				tabindex: "-1",
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("td", {
				...
				tabindex: "-1"
			}, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_1$13);
		};
	}
});
...
var ITableHeaderSelectable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		__expose(expose);
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("th", _hoisted_1$11, [__props.selectable === "multi" ? (openBlock(), createElementBlock("input", {
				key: 0,
				ref: "input",
...
var _hoisted_1$10 = { class: "table-ng__cell table-ng__cell--checkbox" };
...
var ITableCheckbox_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("td", _hoisted_1$10, [createBaseVNode("input", {
				ref: "target",
...
var _hoisted_1$9 = { class: "table-ng__cell table-ng__cell--radio" };
...
var ITableRadio_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("td", _hoisted_1$9, [createBaseVNode("input", {
				ref: "input",
				type: "radio",
				checked: Boolean(__props.column.checked(__props.row)),
				"aria-label": ariaLabel.value,
...
var ITableSelectable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return __props.level > 1 ? (openBlock(), createElementBlock("td", _hoisted_1$8)) : __props.selectable === "multi" ? (openBlock(), createBlock(ITableCheckbox_default, {
				key: 1,
				ref: "child",
				row: __props.row,
...
var ITableAnchor_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return __props.column.text(__props.row) ? (openBlock(), createElementBlock("td", _hoisted_1$7, [createBaseVNode("a", {
				ref: "target",
...
var _hoisted_1$6 = { class: "table-ng__cell table-ng__cell--button" };
...
var ITableButton_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("td", _hoisted_1$6, [createBaseVNode("button", {
				ref: "button",
				class: "icon-button",
				type: "button",
				tabindex: "-1",
				...
				_cache[0] || (_cache[0] = createTextVNode()),
				createBaseVNode("span", _hoisted_2$4, toDisplayString(__props.column.text(__props.row)), 1)
			], 512)]);
		};
	}
});
...
var _hoisted_1$5 = { class: "sr-only" };
		...
		return (_ctx, _cache) => {
			...
			return openBlock(), createElementBlock("td", { class: normalizeClass(["table-ng__cell table-ng__cell--button", { "table-ng__cell--menu-open": isOpen.value }]) }, [
				createBaseVNode("button", {
					...
					_cache[0] || (_cache[0] = createTextVNode()),
					createBaseVNode("span", _hoisted_1$5, toDisplayString(__props.column.text(__props.row)), 1)
				], 512),
				_cache[1] || (_cache[1] = createTextVNode()),
...
var _hoisted_1$4 = { class: "table-ng__editable" };
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return __props.column.editable(__props.row) ? (openBlock(), createElementBlock("td", {
				...
				withDirectives(createBaseVNode("div", _hoisted_1$4, [createBaseVNode("span", _hoisted_2$3, toDisplayString(viewValue.value), 1)], 512), [[vShow, !editing.value]]),
				_cache[3] || (_cache[3] = createTextVNode()),
				...
				}, [createBaseVNode("span", _hoisted_4$3, toDisplayString(viewValue.value), 1)], 40, _hoisted_3$3), [[vShow, editing.value]]),
				_cache[4] || (_cache[4] = createTextVNode()),
		...
		return (_ctx, _cache) => {
			return __props.column.editable(__props.row) ? (openBlock(), createElementBlock("td", {
				...
				createBaseVNode("div", { class: normalizeClass(divClasses.value) }, [
					createBaseVNode("span", _hoisted_2$2, toDisplayString(fromColumnValue()), 1),
					_cache[1] || (_cache[1] = createTextVNode()),
			...
			], 42, _hoisted_1$3)) : (openBlock(), createElementBlock("td", {
				key: 1,
				ref: "td",
				tabindex: "-1",
...
function useSelectable(options) {
	...
	function toggleSelectableHeader() {
		...
		else selectedRows.value = [...toValue(rows)];
	}
...
function useTabstop(tableRef, metaRows) {
	...
	watch(metaRows, (newRows, oldRows) => {
		...
		if (oldTabstopTr.rowIndex === 1) {
			...
			if (newRows.some(matching(needle))) {
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("table", {
				...
				_cache[4] || (_cache[4] = createTextVNode()),
				hasColumns.value ? (openBlock(), createElementBlock("thead", _hoisted_3$1, [createBaseVNode("tr", _hoisted_4$1, [
				...
				_cache[5] || (_cache[5] = createTextVNode()),
				hasColumns.value ? (openBlock(), createElementBlock("tbody", _hoisted_6$1, [isEmpty.value ? (openBlock(), createElementBlock("tr", _hoisted_7$1, [createBaseVNode("td", {
					colspan: fullColspan.value,
					class: "table-ng__cell"
				}, [renderSlot(_ctx.$slots, "empty", {}, () => [createTextVNode(toDisplayString(unref($t)("fkui.ftable.empty.text", "Tabellen är tom")), 1)])], 8, _hoisted_8)])) : (openBlock(true), createElementBlock(Fragment, { key: 1 }, renderList(metaRows.value, ({ key, row, rowIndex, level, setsize, posinset, isExpandable, isExpanded }) => {
				...
				_cache[6] || (_cache[6] = createTextVNode()),
				hasFooter.value ? (openBlock(), createElementBlock("tfoot", _hoisted_10, [createBaseVNode("tr", {
					class: "table-ng__row",
...
function removeRow(rows, row, expandableAttribute) {
	...
	if (rowIndex !== -1) return rows.toSpliced(rowIndex, 1);
	else if (expandableAttribute) return removeExpandableRowFromRows(rows, row, expandableAttribute);
	else return rows;
}
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [createVNode(unref(FSelectField_default), {
				...
				striped: "",
				selectable: "multi",
				"expandable-attribute": expandableAttr.value
			}, createSlots({
			...
			return openBlock(), createElementBlock(Fragment, null, [
				...
				createVNode(unref(FSortFilterDataset_default), {
					...
					default: withCtx(({ sortFilterResult }) => [createVNode(unref(FTable_default), {
						...
						caption: withCtx(() => [..._cache[1] || (_cache[1] = [createTextVNode("Tabell", -1)])]),
						footer: withCtx(() => [..._cache[2] || (_cache[2] = [createTextVNode("Footer", -1)])]),
						_: 1
					}, 8, [
						"selected-rows",
						"rows",
