...
var require_es_iterator_filter$1 = /* @__PURE__ */ __commonJSMin$1((() => {
	var $ = require_export$1();
	var call = require_function_call$1();
	var aCallable = require_a_callable$1();
	var anObject = require_an_object$1();
...
var NavigationView_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const withoutExpandableRows = useDatasetRef(sourceRows.map(({ expandableRows, ...attrs }) => ({ ...attrs })));
		const withExpandableRows = useDatasetRef(sourceRows.map(({ expandableContent, ...attrs }) => ({ ...attrs })), "expandableRows");
		const rows = computed(() => expandableAttr.value === "expandableRows" ? withExpandableRows.value : withoutExpandableRows.value);
