...
import { defineComponent, useTemplateRef, provide, computed, createElementBlock, openBlock, createCommentVNode, createTextVNode, renderSlot, Fragment, createElementVNode, createVNode, unref, toDisplayString, withModifiers, createBlock, nextTick, inject, ref, onMounted, normalizeClass, withDirectives, vModelText, watchEffect, vShow, withCtx, mergeModels, useModel, watch, onUpdated, useSlots, renderList, mergeProps, resolveDynamicComponent } from "vue";
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return renderButton.value ? (openBlock(), createElementBlock("td", {
        ...
        type: "button",
        tabindex: "-1",
        onClick: withModifiers(onClickButton, ["prevent", "stop"])
...
var _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    watch(metaRows, (newRows, oldRows) => {
      assertRef(tableRef);
      const tabstopElement = tableRef.value.querySelector(`[tabindex="0"]`);
      assertSet(tabstopElement);
      const isLosingFocus = tabstopElement === document.activeElement;
      if (newRows.length === 0) {
        renderOptions.value.focus = isLosingFocus;
        renderOptions.value.fallbackToFirstCell = true;
        return;
      }
      if (__props.tabFallback === "first-cell") {
        fallbackToFirstCell(newRows, oldRows, isLosingFocus);
        return;
      }
      const tabstopCol = tabstopElement.closest("td");
      assertSet(tabstopCol);
      const tabstopRow = tabstopCol?.parentElement;
      const tabstopRowKey = oldRows[tabstopRow.rowIndex - 1].key;
      const isBeingRemoved = !newRows.some((it) => it.key === tabstopRowKey);
      if (!isBeingRemoved) {
        return;
      }
      if (tabstopRow.rowIndex === 1) {
        ...
        if (hasRowBelowInNewRows) {
          const {
            ...
            row: 1,
            cell: tabstopCol.cellIndex
          }, {
            row: 2,
            cell: tabstopCol.cellIndex
          });
          const fallback = getCellTarget(tableRef.value, 2, cell);
          dispatchActivateCellEvent(fallback, {
            focus: isLosingFocus
          });
        } else {
          fallbackToFirstCell(newRows, oldRows, isLosingFocus);
        ...
        const hasRowAboveInNewRows = newRows.some((it) => it.key === oldRows[tabstopRow.rowIndex - 2].key);
        if (hasRowAboveInNewRows) {
          const {
            row,
            cell
          } = getVerticalNavIndex(tableRef.value, {
            row: tabstopRow.rowIndex,
            cell: tabstopCol.cellIndex
          }, {
            row: tabstopRow.rowIndex - 1,
            cell: tabstopCol.cellIndex
          });
...
import { openBlock as _openBlock4, createBlock as _createBlock, createCommentVNode as _createCommentVNode3, toDisplayString as _toDisplayString2, createElementVNode as _createElementVNode4, withModifiers as _withModifiers, createElementBlock as _createElementBlock4 } from "vue";
    ...
    [
      _createElementVNode4(
        "button",
        {
          ...
          type: "button",
          tabindex: "-1",
          onClick: _withModifiers($setup.onClickButton, ["prevent", "stop"])
        },
        [
...
import { assertRef as assertRef3, ValidationService as ValidationService2 } from "@fkui/logic";
...
import { toDisplayString as _toDisplayString3, createElementVNode as _createElementVNode5, vModelText as _vModelText, normalizeClass as _normalizeClass, withDirectives as _withDirectives, openBlock as _openBlock5, createBlock as _createBlock2, withModifiers as _withModifiers2, createElementBlock as _createElementBlock5 } from "vue";
...
import { assertRef as assertRef4, assertSet as assertSet2, ElementIdService as ElementIdService2 } from "@fkui/logic";
...
import { toDisplayString as _toDisplayString4, createElementVNode as _createElementVNode6, createVNode as _createVNode, vShow as _vShow, withDirectives as _withDirectives2, withModifiers as _withModifiers3, openBlock as _openBlock6, createElementBlock as _createElementBlock6 } from "vue";
...
function render6(_ctx, _cache, $props, $setup, $data, $options) {
  return $props.column.editable($props.row) ? (_openBlock6(), _createElementBlock6(
    "td",
    {
      ...
      onClick: _withModifiers3($setup.onCellClick, ["stop"]),
      onTableActivateCell: $setup.onActivateCell
    },
    [
      ...
      _withDirectives2(_createElementVNode6("div", {
        ...
        onDblclick: _cache[1] || (_cache[1] = _withModifiers3(() => {
        }, ["prevent"])),
        onKeydown: _withModifiers3($setup.onEditKeyDown, ["stop"]),
