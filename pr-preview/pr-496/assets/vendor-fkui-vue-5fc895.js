...
function useYearSelector(options) {
  ...
  const selectableYears = computed(() => {
    const selectableYears2 = [];
    for (let year = minDate.value.year; year <= maxDate.value.year; year++) {
      selectableYears2.push(year);
    }
    return selectableYears2;
  });
  async function onUpdateDisplayYearSelectorAsOpen(showYearSelector) {
    if (showYearSelector) {
      height.value = getHTMLElementFromVueRef(options.calendar.value).getBoundingClientRect().height;
    }
  ...
  function onYearSelectorKeyDown(event) {
    ...
    switch (event.key) {
      case "ArrowUp":
        event.preventDefault();
        activeYear.value = activeYear.value === firstYear ? lastYear : --activeYear.value;
        updateYearSelectorFocus();
        break;
      case "ArrowDown":
        event.preventDefault();
        activeYear.value = activeYear.value === lastYear ? firstYear : ++activeYear.value;
        updateYearSelectorFocus();
        break;
      case "Enter":
      case " ":
  ...
  function selectYear() {
    let selectedDate = options.modelValue.value.addYears(activeYear.value - currentYear.value);
  ...
  function closeYearSelector() {
    ...
    const calendarNavbar = getHTMLElementFromVueRef(options.calendarNavbar.value);
    const calendarNavbarButton = calendarNavbar.querySelector("button");
    focus$1(calendarNavbarButton);
  }
  return {
...
var _hoisted_2$D = ["aria-activedescendant"];
var _hoisted_3$u = ["id", "tabindex", "aria-selected", "aria-labelledby", "onClick"];
function _sfc_render$C(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return openBlock(), createElementBlock("div", _hoisted_1$R, [createVNode(_component_i_calendar_navbar, {
  ...
  }, null, 8, ["id", "model-value", "min-date", "max-date", "year-selector", "display-year-selector-as-open", "onUpdate:modelValue", "onUpdate:displayYearSelectorAsOpen"]), _cache[1] || (_cache[1] = createTextVNode()), _ctx.displayYearSelectorAsOpen ? (openBlock(), createElementBlock("div", {
    key: 0,
    ref: "yearSelector",
    ...
    return openBlock(), createElementBlock("li", {
      ...
      key: year,
      role: "option",
      tabindex: _ctx.activeYear === year ? 0 : -1,
      "aria-selected": _ctx.activeYear === year ? "true" : void 0,
      "aria-labelledby": _ctx.getSelectableYearId(year),
      class: normalizeClass(["calendar__year-selector__year", {
        "calendar__year-selector__year--highlight": _ctx.activeYear === year
      }]),
      onKeydown: _cache[0] || (_cache[0] = (...args) => _ctx.onYearSelectorKeyDown && _ctx.onYearSelectorKeyDown(...args)),
