...
async function importIcons() {
  const icons = importDefault(await Promise.resolve().then(() => (init_dist(), dist_exports)));
...
function initDocsViewSwitcher() {
  ...
  function getView() {
    ...
    const targetView = target?.closest("[data-docs-view]")?.dataset.docsView;
    if (targetView) {
      return targetView;
    }
  ...
  function showView(view) {
    ...
    for (const link of viewLinks) {
      const ariaCurrent = link.dataset.docsViewLink === view ? "page" : "false";
