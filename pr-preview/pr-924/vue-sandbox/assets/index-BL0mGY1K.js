...
/* @__PURE__ */ defineComponent({
  ...
  setup(__props, {
    ...
    function handleDataChanged() {
      const newData = filter(props.data, internalFilterAttributes.value, searchString.value);
      const noLongerIncluded = sortFilterResult.value.filter((it) => !newData.includes(it));
      noLongerIncluded.forEach((it) => sortFilterResult.value.splice(sortFilterResult.value.indexOf(it)));
      const additions = newData.filter((it) => !sortFilterResult.value.includes(it));
      additions.forEach((it) => sortFilterResult.value.push(it));
      sortAttribute.value = defaultSortValue;
      tableCallbackOnSort(defaultSortValue.attribute, defaultSortValue.ascending);
      emit2("datasetSorted", sortFilterResult.value);
    }
    function handleDataReplaced() {
