...
function removeRow(rows, row, expandableAttribute) {
	...
	else if (expandableAttribute) removeExpandableRowFromRows(rows, row, expandableAttribute);
}
function removeExpandableRowFromRows(rows, row, key) {
	for (const currentRow of rows) {
		...
		if (Array.isArray(expandableRows)) {
			...
			if (index !== -1) expandableRows.splice(index, 1);
		}
	}
}
/**
* Removes given rows from the array.
* If `expandableAttribute` is set it can removes rows from nested rows.
*
* Note: The array is mutated.
*
...
* @since %version%
* @param rows - The row array
* @param removeRows - The rows to remove (expected to exist in the row array)
* @param expandableAttribute - If set, this function will search for and remove expandable rows the given row.
*/
function removeRows(rows, removeRows, expandableAttribute) {
	for (const it of removeRows) removeRow(rows, it, expandableAttribute);
}
	...
	setup(__props) {
		...
		function onRemoveRow(row) {
			...
			tableRef.value.withTabstopBehaviour("row-removal", () => {
				removeRow(rows.value, row, "expandableRows");
			});
		}
		function onRemoveSelectedRows() {
			removeRows(rows.value, mySelectedRows.value);
		}
		return (_ctx, _cache) => {
