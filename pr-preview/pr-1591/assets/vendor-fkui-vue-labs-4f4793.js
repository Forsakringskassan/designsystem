...
var require_internal_state = /* @__PURE__ */ __commonJSMin(((exports, module) => {
  ...
  var getterFor = function(TYPE) {
    return function(it) {
      var state;
      if (!isObject(it) || (state = get(it)).type !== TYPE) throw new TypeError2("Incompatible receiver, " + TYPE + " required");
      return state;
    };
  };
...
var _hoisted_2 = { class: "error-icon" };
...
var XFileDragdrop_default = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    function drop(e) {
      ...
      visadrag.value = false;
      dragEnterTarget.value = null;
      if (e.dataTransfer) hanteraFil(e.dataTransfer.files);
    }
    function hanteraFil(filer) {
      filValidering.value = "";
      if (filer.length > 1) filValidering.value = "Du kan bara ladda upp ett dokument per tillf\xE4lle";
      else if (filer[0].type === "" || filer[0].type !== "" && !allowedFileTypes.value.includes(filer[0].type)) filValidering.value = "Dokumentet har fel format, konvertera dokumentet till en pdf";
      else if (filer[0].size > maxFileSize.value) filValidering.value = "Dokumentet \xE4r f\xF6r stort (\xF6ver 10 MB)";
        ...
        Object.values(filer !== null && filer !== void 0 ? filer : valdFil.value).forEach(async (value) => {
          try {
            ...
            uppladdatDokument.value = { dokument: {
              filnamn: value.name,
              mime: value.type,
              data: btoa(reduced),
              fel: ""
      ...
      if (filValidering.value) skickaFel(filer);
    }
    ...
    return (_ctx, _cache) => {
      ...
      return openBlock(), createElementBlock("div", {
        ...
        _cache[7] || (_cache[7] = createElementVNode("h3", null, "Ladda upp ett dokument", -1)),
        _cache[8] || (_cache[8] = createTextVNode("\n        dra och sl\xE4pp eller\n        ", -1)),
        filValidering.value !== "" ? (openBlock(), createElementBlock("div", _hoisted_1$2, [createElementVNode("span", _hoisted_2, [createVNode(unref(FIcon), { name: "error" })]), createTextVNode(" " + toDisplayString(filValidering.value), 1)])) : createCommentVNode("", true),
        _cache[9] || (_cache[9] = createTextVNode()),
        createElementVNode("div", null, [withDirectives((openBlock(), createBlock(unref(FFileSelector), {
          id: "valjFil",
          ...
          "accept",
          "aria-label"
        ])), [[_directive_test, "valdFil"]])]),
        _cache[10] || (_cache[10] = createTextVNode()),
        harValtFil.value || harMisslyckats.value ? (openBlock(), createElementBlock("div", _hoisted_3, [
          _cache[4] || (_cache[4] = createElementVNode("hr", { class: "file-item__separator show_separator" }, null, -1)),
          _cache[5] || (_cache[5] = createTextVNode()),
          withDirectives((openBlock(), createBlock(unref(FFileItem), {
            id: "fillista",
            ...
            row: withCtx(() => [createElementVNode("span", _hoisted_4, [
              createElementVNode("span", _hoisted_5, toDisplayString(fileName.value) + " " + toDisplayString(filstorlek.value) + " kb ", 1),
...
function timeAgo(timestamp, reference, $t) {
  ...
  if (seconds < 60) return $t("fkui.time-ago.seconds", "{{ count }}s ago", { count: seconds });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return $t("fkui.time-ago.minutes", "{{ count }}m ago", { count: minutes });
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return $t("fkui.time-ago.hours", "{{ count }}h ago", { count: hours });
  const days = Math.floor(hours / 24);
  if (days < 30) return $t("fkui.time-ago.days", "{{ count }}d ago", { count: days });
  const months = Math.floor(days / 30);
  if (months < 12) return $t("fkui.time-ago.months", "{{ count }}mo ago", { count: months });
  return $t("fkui.time-ago.years", "{{ count }}y ago", { count: Math.floor(months / 12) });
}
var _hoisted_1$1 = ["datetime", "title"];
var XRelativeTime_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
...
var XRelativeTime_default = /* @__PURE__ */ _plugin_vue_export_helper_default(XRelativeTime_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-03b0dfe5"]]);
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        renderSlot(_ctx.$slots, "filter", normalizeProps(guardReactiveProps({ update: unref(debouncedUpdate) }))),
        _cache[0] || (_cache[0] = createTextVNode()),
        renderSlot(_ctx.$slots, "default", normalizeProps(guardReactiveProps({ result: result.value })))
      ]);
    };
  }
});
...
function minutesToHoursFloat(...values) {
  return values.filter((value) => isSet(value) && !Number.isNaN(value)).reduce((sum, value) => sum + value, 0) / 60;
}
...
export {
  ...
  XFileDragdrop_default as XFileDragdrop,
  XRelativeTime_default as XRelativeTime,
  XSortFilterDatasetNg_default as XSortFilterDatasetNg,
  XTimeTextField_default as XTimeTextField,
