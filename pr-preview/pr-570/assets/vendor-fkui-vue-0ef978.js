...
import { defineComponent, computed, createElementBlock, openBlock, normalizeClass, renderSlot, mergeProps, createTextVNode, createElementVNode, ref, useAttrs, useSlots, unref, createCommentVNode, Fragment, createBlock, createApp, resolveComponent, withKeys, createVNode, toDisplayString, withCtx, renderList, withModifiers, isVNode, Comment, getCurrentInstance, resolveDynamicComponent, capitalize, onMounted, toValue, onUnmounted, normalizeProps, guardReactiveProps, Transition, Teleport, normalizeStyle, useTemplateRef, watchEffect, watch, nextTick, withDirectives, vShow, readonly, inject, toRef, provide, createSlots, vModelSelect, vModelDynamic, toHandlers, shallowRef, getCurrentScope, onScopeDispose, hasInjectionContext, defineCustomElement, effectScope, onUpdated, toRefs } from "vue";
...
var _sfc_main$1l = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const props = __props;
    function hasIconLeft() {
      return !!slots["iconLeft"];
    }
    function hasIconRight() {
      return !!slots["iconRight"];
    }
    function hasIcon() {
      return hasIconLeft() || hasIconRight();
    }
    const buttonClass = computed(() => {
      let classes = `button button--${props.type} button--${props.size}`;
      if (props.type === "tertiary") {
        classes = `${classes} button--tertiary--${props.tertiaryColor}`;
      }
      if (props.mobileFullWidth) {
        classes = `${classes} button--full-width`;
      }
      if (!hasIcon() && !inflight.value) {
        return `${classes} button--no-icon`;
      }
      return classes;
    });
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("button", mergeProps({
      ...
      }, attrs), [hasIconLeft() ? (openBlock(), createElementBlock(Fragment, {
        key: 0
        ...
        key: 1
      })], 64)) : createCommentVNode("", true), _cache[0] || (_cache[0] = createTextVNode()), !hasIcon() ? (openBlock(), createElementBlock(Fragment, {
        key: 1
      }, [unref(inflight) ? (openBlock(), createBlock(unref(FIcon), {
        key: 0,
        name: "circle-notch-solid",
        class: "button__icon button__inflight"
      })) : createCommentVNode("", true)], 64)) : createCommentVNode("", true), _cache[1] || (_cache[1] = createTextVNode()), renderSlot(_ctx.$slots, "default"), _cache[2] || (_cache[2] = createTextVNode()), hasIconRight() ? (openBlock(), createElementBlock(Fragment, {
        key: 2
      }, [unref(inflight) ? (openBlock(), createBlock(unref(FIcon), {
        key: 0,
        name: "circle-notch-solid",
        ...
        key: 1
      })], 64)) : createCommentVNode("", true)], 16, _hoisted_1$$);
    };
  }
});
