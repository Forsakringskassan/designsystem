...
var FTextareaField_vue_vue_type_script_lang_default = /* @__PURE__ */ defineComponent({
	...
	computed: {
		...
		textareaClass() {
			...
			if (this.autoResize) {
				...
				if (this.normalizedMaxRows) classes.push("textarea-field__resize--max-rows");
			} else if (this.resizable) classes.push("textarea-field__resize--vertical");
		...
		normalizedMaxRows() {
			const { maxRows } = this;
			if (typeof maxRows !== "number" || !Number.isInteger(maxRows) || maxRows < 1) return;
	...
	methods: {
		updateTextareaHeightVariables() {
			...
			if (this.normalizedMaxRows) textarea.style.setProperty("--i-textarea-field-max-height", `${this.normalizedMaxRows}lh`);
			else textarea.style.removeProperty("--i-textarea-field-max-height");
		},
		onInput(event) {
