...
import { FIcon, IFlex, IFlexItem, useTranslate, getItemIdentifier, getRowAriaData, FContextMenu, IComboboxDropdown, IPopupError, dispatchComponentValidityEvent, findItemIdentifier, useSlotUtils, reactiveDataset, setItemIdentifiers, FSortFilterDatasetInjected, EventBus, FFileSelector, FFileItem, TranslationMixin, FTextField, useTextFieldSetup } from "@fkui/vue";
...
function getMetaRows(keyedRows, expandedKeys, expandableAttribute) {
  ...
  walk(keyedRows, expandableAttribute, (row) => {
    ...
    array.push({
      ...
      row,
      rowIndex,
      level: expandableAttribute ? meta.ariaLevel : void 0,
      isExpandable,
      isExpanded
    });
    return isExpanded;
...
var _sfc_main$3 = /* @__PURE__ */ defineComponent2({
  ...
  setup(__props, {
    ...
    const dataset = computed3(() => reactiveDataset(__props.rows, __props.expandableAttribute));
    const keyedRows = computed3(() => setItemIdentifiers(dataset.value, __props.keyAttribute, __props.expandableAttribute));
    const metaRows = computed3(() => getMetaRows(dataset.value, expandedKeys.value, __props.expandableAttribute));
