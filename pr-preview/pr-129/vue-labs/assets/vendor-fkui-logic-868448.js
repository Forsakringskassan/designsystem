...
(() => {
  ...
  __export(esm_exports, {
    ...
    focusLast: () => focusLast,
    formatClearingNumberForBackend: () => formatClearingNumberForBackend,
    ...
    normalizeDateFormat: () => normalizeDateFormat,
    parseBankAccountNumber: () => parseBankAccountNumber,
    parseBankgiro: () => parseBankgiro,
    parseClearingNumber: () => parseClearingNumber,
    parseDate: () => parseDate,
    parseNumber: () => parseNumber,
    parseOrganisationsnummer: () => parseOrganisationsnummer,
    parsePercent: () => parsePercent,
    parsePersonnummer: () => parsePersonnummer,
    parsePersonnummerLuhn: () => parsePersonnummerLuhn,
    parsePlusgiro: () => parsePlusgiro,
    ...
    testLuhnChecksum: () => testLuhnChecksum,
    validChecksum: () => validChecksum,
    validLimit: () => validLimit,
    waitForScreenReader: () => waitForScreenReader
  });
  var configLogic = {
    production: true
  };
...
Caused by: ${cause.stack}`;
  ...
  var DATE_REGEXP_WITH_DASH = /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/;
  ...
  var POSTAL_CODE_REGEXP = /^([1-9]{1}\d{2}) ?(\d{2})$/;
  ...
  var ORGANISATIONSNUMMER_REGEXP = /^([0-9]{6})-?([0-9]{4})$/;
  ...
  var compareAgePnrValue = {
    ...
    validation(value, _element, config) {
      ...
      let otherFieldPnr = parsePersonnummer(String(config.otherField));
      if (!otherFieldPnr) {
        return true;
      }
      ...
      const pnrValueShouldBeOlder = config.pnrValueShouldBeOlder ? -1 : 1;
  ...
  window.__modules__["@fkui/logic"] = esm_exports;
})();
