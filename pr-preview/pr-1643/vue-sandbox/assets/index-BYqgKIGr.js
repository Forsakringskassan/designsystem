...
var vModelSelect = {
	...
	created(el, { value, modifiers: { number } }, vnode) {
		...
		addEventListener(el, "change", () => {
			...
			const assignedValue = multiple ? isSet$1(el._modelValue) ? new Set(selectedVal) : selectedVal : selectedVal[0];
			const pending = el._pendingValue = [multiple, multiple ? isArray$1(assignedValue) ? selectedVal.slice() : selectedVal : assignedValue];
			try {
...
//#endregion
//#region ../../packages/logic/dist/esm/index.js
/**
...
function testLuhnChecksum(inputString) {
	if (!/^\d+$/.test(inputString)) throw new Error("Luhn Checksum test only works on strings containing numbers");
	let sum = 0;
	for (const [index, numChar] of inputString.split("").toReversed().entries()) {
		const digit = Math.trunc(Number(numChar)) * ((index + 1) % 2 === 0 ? 2 : 1);
...
var FDate = class FDate {
	...
	*/
	static fromIso(value) {
		...
		if (match?.groups) {
			...
			if (date.isValid() && date.month === Math.trunc(Number(month))) return date;
		}
		return this.invalid();
	}
	/**
	...
	*/
	static compare(a, b) {
		if (typeof a === "string") a = this.fromIso(a);
		if (typeof b === "string") b = this.fromIso(b);
...
var FieldsetValidationHandler = class {
	...
	onFocusIn(event) {
		if (!this.hasFocusableTarget(event.target) || this.hasDocumentListener) return;
	...
	removeEventListeners() {
		if (!(this.hasDocumentListener && this.documentFocusInRef)) return;
...
var ValidationServiceImpl = class {
	...
	async isValid(src, root = document) {
		function isValidSync(src) {
			if (!src) return false;
			if (Array.isArray(src)) return src.every((it) => isValidSync(it));
			if (typeof src === "string") return isValidSync(root.querySelector(`#${src}`));
			if (isValidatableFormElement(src)) return src.validity.valid;
	...
	getValidatorByName(name) {
		if (!Object.hasOwn(registry, name)) throw new Error(`Validator '${name}' does not exist or is not registered, see ValidatorService.registerValidator.`);
	...
	resolveValidityMode(element, validationState, isValid) {
		...
		if (isValid) return this.resolveValidityModeWhenValid(element);
	...
	validate(value, element, validator, validatorConfigs) {
		...
		*/
		return validatorConfig.enabled === void 0 || validatorConfig.enabled ? validator.validation(value, element, validatorConfig) : true;
	}
...
function createNumberRegexp(minDecimals = 0, maxDecimals = 2) {
	return new RegExp(`^([-\u{2212}]?[0-9]+)([,.][0-9]{${String(minDecimals)},${String(maxDecimals)}})(?<![,.])$`);
}
...
var requiredValidator = {
	...
	validation(value, element) {
		...
		if (isRadiobuttonOrCheckbox(element)) return validateChecked(element);
...
//#endregion
//#region ../../packages/date/dist/esm/index.js
...
var _hoisted_1$81 = ["aria-hidden"];
var _hoisted_2$58 = ["href"];
function _sfc_render$50(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("svg", mergeProps(_ctx.$attrs, {
		...
		createBaseVNode("use", { href: _ctx.spriteId }, null, 8, _hoisted_2$58)
	], 16, _hoisted_1$81);
}
...
function getTextContent(children, options) {
	return children.filter(isVNode).filter(excludeComment).filter(excludeClass(options.stripClasses)).map((node) => {
		if (isComponent(node)) {
			if (options.componentPlaceholder) return `<${getComponentName(node)} />`;
			return "";
		}
...
function registerValidators(el, binding) {
	...
	const target = getValidatableElement(el);
	for (const validatorName of Object.keys(bindingValue)) if (!Object.hasOwn(bindingModifiers, validatorName)) throw new Error(`Have you forget to add '${validatorName}' to v-validation.${validatorName}?`);
...
var require_es_iterator_flat_map = /* @__PURE__ */ __commonJSMin((() => {
	var $ = require_export();
	var call = require_function_call();
	var aCallable = require_a_callable();
	var anObject = require_an_object();
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return __props.isOpen ? (openBlock(), createBlock(Teleport, {
				...
				"aria-hidden": "true"
			}, [createBaseVNode("div", _hoisted_1$67, [createBaseVNode("div", {
			...
			], 6)], 512)], 2)], 8, ["to", "disabled"])) : createCommentVNode("v-if", true);
		};
	}
});
/**
...
var _hoisted_1$66 = ["onKeyup"];
var _hoisted_2$46 = { ref: "content" };
...
var IPopupListbox_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			...
			}), [createBaseVNode("div", _hoisted_2$46, [renderSlot(_ctx.$slots, "default")], 512)], 16, _hoisted_1$66)], 512)], 8, ["to"])) : createCommentVNode("v-if", true);
		};
	}
});
...
var _hoisted_1$62 = { class: "combobox" };
var _hoisted_2$44 = ["id"];
var _hoisted_3$33 = [
	"id",
	"aria-selected",
	"onClick"
...
var IComboboxDropdown_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		watchEffect(async () => {
			if (__props.activeOption === null) return;
			await nextTick();
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$62, [createVNode(unref(IPopupListbox_default), {
				...
				default: withCtx(() => [createBaseVNode("ul", {
					...
					return openBlock(), createElementBlock("li", {
						...
						onMousedown: _cache[0] || (_cache[0] = withModifiers(() => {}, ["stop", "prevent"]))
					}, toDisplayString(item), 43, _hoisted_3$33);
				}), 128))], 8, _hoisted_2$44)]),
				_: 1
			}, 8, [
				"is-open",
...
function _sfc_render$26(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return _ctx.$slots.tooltip ? (openBlock(), createElementBlock("div", _hoisted_1$54, [
		_ctx.hasDefaultSlot ? (openBlock(), createElementBlock("div", _hoisted_2$39, [createBaseVNode("label", {
			class: "label",
			for: _ctx.forProperty
		}, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_3$28)], 512)) : createCommentVNode("v-if", true),
		...
		_ctx.hasDescriptionSlot || _ctx.hasErrorMessageSlot ? (openBlock(), createElementBlock("label", {
			...
			_cache[1] || (_cache[1] = createTextVNode()),
			_ctx.hasErrorMessageSlot ? (openBlock(), createElementBlock("span", _hoisted_5$18, [
		...
		], 8, _hoisted_4$22)) : createCommentVNode("v-if", true)
...
function useTextFieldSetup(props) {
	...
	async function onOptionSelected(value) {
		...
		inputNode.value.dispatchEvent(new CustomEvent("select", {
			bubbles: false,
			detail: viewValue.value
		}));
	}
	...
	methods: {
		...
		syncViewValueAfterModelUpdate(newModelValue) {
			if (newModelValue === "") this.viewValue = "";
				...
				if (isSet(this.formatter)) this.viewValue = this.formatter(newModelValue) || this.viewValue;
...
function _sfc_render$24(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createElementBlock("div", { class: normalizeClass(["text-field", _ctx.rootClass]) }, [
		createBaseVNode("div", { class: normalizeClass(_ctx.labelWrapperClass) }, [createVNode(_component_f_label, {
			...
			default: withCtx(() => [renderSlot(_ctx.$slots, "default", {}, () => [_ctx.defaultText !== "" ? (openBlock(), createElementBlock("span", _hoisted_1$38, toDisplayString(_ctx.defaultText), 1)) : createCommentVNode("v-if", true)])]),
			description: withCtx(({ descriptionClass, formatDescriptionClass }) => [renderSlot(_ctx.$slots, "description", {
				...
				_ctx.descriptionText ? (openBlock(), createElementBlock("span", {
					...
					_ctx.descriptionScreenReaderText ? (openBlock(), createElementBlock("span", _hoisted_2$29, toDisplayString(_ctx.descriptionScreenReaderText), 1)) : createCommentVNode("v-if", true),
				...
				_ctx.discreteDescriptionText ? (openBlock(), createElementBlock("span", {
					...
					_ctx.discreteDescriptionScreenReaderText ? (openBlock(), createElementBlock("span", _hoisted_3$23, toDisplayString(_ctx.discreteDescriptionScreenReaderText), 1)) : createCommentVNode("v-if", true),
			...
			})), () => [_ctx.hasError ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [createTextVNode(toDisplayString(_ctx.validationMessage), 1)], 64)) : createCommentVNode("v-if", true)])]),
			_: 2
		}, [_ctx.$slots.tooltip ? {
			name: "tooltip",
		...
		createBaseVNode("div", { class: normalizeClass(["text-field__input-wrapper", _ctx.inputWrapperClass]) }, [
			...
			createBaseVNode("div", _hoisted_4$18, [
				withDirectives(createBaseVNode("input", mergeProps({
					...
					onSelect: _cache[1] || (_cache[1] = (...args) => _ctx.onInputSelect && _ctx.onInputSelect(...args)),
					...
					onValidationConfigUpdate: _cache[5] || (_cache[5] = (...args) => _ctx.onValidationConfigUpdate && _ctx.onValidationConfigUpdate(...args)),
					onValidity: _cache[6] || (_cache[6] = (...args) => _ctx.onValidity && _ctx.onValidity(...args)),
					onPendingValidity: _cache[7] || (_cache[7] = (...args) => _ctx.onPendingValidity && _ctx.onPendingValidity(...args))
				...
				_ctx.$slots["append-inner"] ? (openBlock(), createElementBlock("div", _hoisted_6$11, [renderSlot(_ctx.$slots, "append-inner")])) : createCommentVNode("v-if", true),
