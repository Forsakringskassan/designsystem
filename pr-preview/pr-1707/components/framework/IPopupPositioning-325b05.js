...
// virtual-entry:virtual:packages/vue/src/internal-components/IPopup/examples/IPopupPositioning.vue:IPopupPositioning-325b05.js
...
import { DateFormat as DateFormat2, FDate as FDate3 } from "@fkui/date";
import {
...
// packages/vue/src/plugins/format/date-range-to-string.ts
import { DateFormat, FDate } from "@fkui/date";

// packages/vue/src/plugins/format/is-date-range.ts
import { FDate as FDate2 } from "@fkui/date";
...
function fitInsideArea(options) {
  if (isElementOptions(options)) {
    const {
      ...
      spacing: spacing2,
      candidateOrder,
      allowAnchorOverlap: allowAnchorOverlap2
    } = options;
  ...
  const index = candidates.findIndex(
    (it) => isInside(clippedArea, it, spacing) && (allowAnchorOverlap || !isOverlapping(anchor, it))
  );
  if (index !== -1) {
