...
function getHorizontalScrollParent(element) {
	let parent = element.parentElement;
	while (parent) {
		const style = window.getComputedStyle(parent);
		if ((style.overflowX === "auto" || style.overflowX === "scroll") && parent.scrollWidth > parent.clientWidth) return parent;
...
function ensureCellVisible(cell) {
	...
	if (scrollParent) {
		if (isHorizontallyVisibleWithinContainer(cell, scrollParent)) return;
	} else if (isHorizontallyVisibleWithinViewport(cell)) return;
	cell.scrollIntoView({
		block: "nearest",
		inline: "nearest"
	});
