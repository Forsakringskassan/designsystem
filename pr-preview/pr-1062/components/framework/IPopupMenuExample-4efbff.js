...
// virtual-entry:virtual:packages/vue/src/internal-components/IPopupMenu/examples/IPopupMenuExample.vue:IPopupMenuExample-4efbff.js
...
var IPopup_default = defineComponent12({
  ...
  methods: {
    ...
    async calculatePlacement() {
      ...
      const anchor = "el" in rawAnchor ? rawAnchor.el : rawAnchor;
      ...
      if (shouldCheckCandidates) {
        ...
        const containerElement = rawContainer && "el" in rawContainer ? rawContainer.el : rawContainer;
        const area = getContainer(popup, containerElement);
        const rawViewport = getElement(this.viewport);
        const viewport = rawViewport && "el" in rawViewport ? rawViewport.el : rawViewport ?? void 0;
        const result = fitInsideArea({
          area,
          anchor,
          target: wrapper,
