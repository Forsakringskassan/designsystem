...
describe("validation", () => {
    ...
    it.each`
        ...
        ${"-3"}          | ${true}  | ${testConfig}            | ${"value below limit should be valid"}
        ...
        ${"3,3"}         | ${true}  | ${{ limit: "3,4" }}      | ${"value with comma(,) seperator should be valid"}
        ${"3.3"}         | ${true}  | ${{ limit: "3.4" }}      | ${"value with dot(.) seperator should be valid"}
    ...
    `(
        ...
        ({ value, expected, config }) => {
            expect(
                lessThanValidator.validation(value, element, config),
            ).toEqual(expected);
        },
    );
