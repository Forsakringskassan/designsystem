...
import { useAreaData } from "../../components/FPageLayout";
...
const layoutElement = computed(() => {
    return root.value?.closest<HTMLElement>("ce-page-layout") ?? undefined;
});
...
<template>
    <div ref="root" class="resize" :class="`resize--${attachment}`">
        <div ref="content" class="resize__content">
            <slot v-bind="{ min: state.min, max: state.max, current: state.current }"></slot>
        </div>
        <div
            v-if="!props.disabled"
            ref="separator"
