...
export default defineComponent({
    ...
    computed: {
        ...
        changeYearContextMenuItems(): ContextMenuItem[] {
            const currentYear = this.modelValue.year;
            return [
                { label: `${currentYear - 2}`, key: "-2" },
                { label: `${currentYear - 1}`, key: "-1" },
                { label: `${currentYear}`, key: "0" },
                { label: `${currentYear + 1}`, key: "1" },
                { label: `${currentYear + 2}`, key: "2" },
            ];
        },
    ...
    <div class="calendar-navbar">
        <div class="calendar-navbar__month" tabindex="-1">
            ...
            <button
                ...
                type="button"
                aria-haspopup="menu"
                @click="onClickYearSelector"
            >
                <f-icon :name="isYearSelectorOpen ? `close` : `arrow-down`" />{{ null }}
            </button>
        </div>
