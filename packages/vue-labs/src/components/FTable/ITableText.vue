...
import { type ValidityEvent, type ValidityNativeEvent, ValidationService, assertRef } from "@fkui/logic";
import { FIcon, IPopupError, dispatchComponentValidityEvent } from "@fkui/vue";
...
function setUpValidation(el: HTMLInputElement): void {
    ...
    void nextTick().then(() => ValidationService.validateElement(el)); // wait for .value to be set before validation
}
