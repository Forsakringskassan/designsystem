...
function testLuhnChecksum(inputString) {
  ...
  let sum = 0;
  for (const [index2, numChar] of inputString.split("").toReversed().entries()) {
    const digit = Math.trunc(Number(numChar)) * ((index2 + 1) % 2 === 0 ? 2 : 1);
...
var ValidationServiceImpl = class {
  ...
  async isValid(src, root = document) {
    function isValidSync(src2) {
      ...
      if (isValidatableFormElement(src2)) {
  ...
  validate(value, element, validator, validatorConfigs) {
    ...
    const isEnabled = validatorConfig.enabled === void 0 || validatorConfig.enabled;
